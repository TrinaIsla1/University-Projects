...
export default function EvolutionChain({ stages }) {
  ...
  if (!stages || stages.length <= 1) {
    return (
      <Text style={[styles.empty, { color: theme.colors.subtext }]}>Does not evolve</Text>
    );
  }
          ...
          {i < stages.length - 1 && (
            <View style={styles.arrowWrap}>
              ...
              {describeTransition(stages[i + 1]) && (
                <Text style={[styles.trigger, { color: theme.colors.subtext }]} numberOfLines={2}>
                  {describeTransition(stages[i + 1])}
                </Text>
              )}
            </View>
...
const styles = StyleSheet.create({
  ...
  stage: { paddingVertical: 6, paddingHorizontal: 10, borderRadius: 8, backgroundColor: 'rgba(120,120,140,0.12)' },
